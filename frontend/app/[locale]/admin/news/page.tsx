...
import { api } from "../../../../lib/api"; // Adjust this path to wherever your api.ts is located

// This defines the shape of the data we expect from NestJS
interface NewsItem {
  id: string;
  title: string;
  slug: string;
...
export default function NewsAdminPage() {
  ...
  const fetchNews = async () => {
    try {
      setIsLoading(true);
      // We pass ?all=true so the admin sees drafted/unpublished posts too
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      // Remove the deleted item from the screen immediately without reloading
  ...
  // Format the ISO date into something readable (DD.MM.YYYY)
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Sitede yayınlanan tüm haberleri ve duyuruları buradan yönetin.</p>
        </div>
        <Link
          href="/tr/admin/news/create"
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-12 text-center text-red-500">
            <p>{error}</p>
            <button onClick={fetchNews} className="mt-4 underline hover:text-red-600">Tekrar Dene</button>
          </div>
        ) : news.length === 0 ? (
                  ...
                  <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          href={`/tr/admin/news/edit/${item.id}`}
